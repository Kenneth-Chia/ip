...
4. Run the bundled generator from the repository root with Python 3. In Codex
   desktop, use the Python executable reported by `load_workspace_dependencies`
   when Python is not on `PATH`:

   ```bash
   <python-3> .agents/skills/present-changes-visually/scripts/generate-split-view-diff.py \
